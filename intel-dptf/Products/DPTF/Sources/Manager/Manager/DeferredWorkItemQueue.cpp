...
XmlNode* DeferredWorkItemQueue::getXml(void) const
{
    ...
    deferredQueueStastics->addChild(XmlNode::createDataElement("current_count", GccFix::to_string(m_queue.size())));
    deferredQueueStastics->addChild(XmlNode::createDataElement("max_count", GccFix::to_string(m_maxCount)));
