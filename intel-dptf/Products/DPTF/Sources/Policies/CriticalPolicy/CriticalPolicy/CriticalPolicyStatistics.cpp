...
XmlNode* CriticalPolicyStatistics::getXml()
{
    ...
    node->addChild(XmlNode::createDataElement("sleep_signaled", GccFix::to_string(m_numberOfTimesSleepSignalled)));
    node->addChild(XmlNode::createDataElement("hibernate_signaled", GccFix::to_string(m_numberOfTimesHibernateSignalled)));
    node->addChild(XmlNode::createDataElement("shutdown_signaled", GccFix::to_string(m_numberOfTimesShutdownSignalled)));
    return node;
}
